...
- 目前工作基準：分支 `codex/detail-history-sync`，前次 GitHub `main` checkpoint `2f03675`（跨 IP 商品切換修正；Render Live，35.2 秒）。本輪更新商品 API 的來源與圖片覆蓋資訊，沒有更動 schema；每次推送後須重新檢查 Render 部署狀態。
