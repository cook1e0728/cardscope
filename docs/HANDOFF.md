...
- 目前工作基準：分支 `codex/detail-history-sync`，前次 GitHub `main` checkpoint `a4f3020`。本輪 SV9／SV10 只使用既有私有候選區與交易式升級函式，沒有更動程式或 schema；每次推送後須重新檢查 Render 部署狀態。
...
下一個安全起點是唯讀預檢 SV9a `001–092`：正式庫目前 92 張稀有度缺值、圖片已全有；來源抽樣 `001`、`046` 有可辨識稀有度，但 `092` 回傳 `None`，不可直接映射為既有稀有度。先全批核對 exact provider ID、官方繁中名稱、版本、來源稀有度及受支援映射；未通過的卡保持空值。每批最多 100 個候選。S12 的 20 張、S10D 的 67 張及 S10b 繼續視為 `source-field-missing`；來源未補欄位前不推測。S12 仍有 94 張缺可靠圖片，不借用其他 printing。
