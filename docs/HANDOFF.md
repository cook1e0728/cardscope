...
- 目前工作基準：分支 `codex/detail-history-sync`、程式 commit `769750d`。S12 使用既有私有候選區與交易式升級函式，沒有更動 schema；本次交接文件更新不更動程式。Render 最近一次已知驗收為此程式版本 Live、`/api/catalog/health` HTTP 200；每次推送後須重新檢查部署狀態。
...
- 已完成系列：S10a `001–071`、S10P `001–067`、S11 `001–100` 與 S11a `001–068` 的 card／printing 稀有度均已補齊，且官方繁中名稱均完成逐張核對。
...
下一個寫入目標尚待重新盤點，暫不預設系列或建立候選。先唯讀複查 S12 上述 20 個來源缺稀有度項目是否有新資料，並重新核對 S10D 待預檢 67 張及其他 Pokémon 系列的來源欄位與正式庫缺口；確認優先序後再分批執行。只有來源逐張提供官方繁中名稱與非空稀有度、正式目標仍空白且 exact provider ID 唯一時，才可建立候選並升級。每批最多 100 個候選。S12 仍有 94 張缺可靠圖片；未取得同一 printing 的可靠圖片來源前維持缺圖。S10b 維持 `source-field-missing`，除非同一來源後續補回欄位或另有來源完成治理審核，不得直接升級。
