...
async function selectGame(id){const choice=gameChoices.find(g=>g.id===id)||gameChoices[0];switchButton.textContent=choice.name;close();if(choice.query){document.getElementById('q').value=choice.query;document.getElementById('go').click()}else originalChoose?.(id)}
