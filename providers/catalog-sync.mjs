import { normalizeCardNameRecord, normalizeRarityRecord, normalizeRarityValue } from './normalize.mjs';
...
const rarityRow=(gameId,value,{source,sourceUrl,metadata={}}={})=>{
  ...
  const normalized=normalizeRarityValue(gameId,evidence.code);
  return normalizeRarityRecord({gameId,rarityCode:evidence.code,rarityLabel:evidence.label,rawRarity:evidence.raw,source,sourceUrl,dataStatus:normalized.known||evidence.label!==evidence.code?'verified':'incomplete',metadata:{...metadata,rawRarity:evidence.raw,labelStatus:evidence.label===evidence.code?'source-value':'source-labeled'}});
};
const rarityMetadata=(gameId,value,metadata={})=>{
  const normalized=normalizeRarityValue(gameId,value);
  return {...metadata,rawRarity:normalized.raw,normalizedRarityCode:normalized.code,normalizedRarityLabel:normalized.label,rarityStatus:normalized.known?'mapped':'unmapped'};
};
const rarityFields=(gameId,value)=>{const normalized=normalizeRarityValue(gameId,value);return {rarity:normalized.raw,rarity_code:normalized.code,rarity_label:normalized.label};};
const cardRarityFields=(gameId,value)=>{const normalized=normalizeRarityValue(gameId,value);return {rarity:normalized.raw,rarity_tier:normalized.rank};};
...
const rowId=row=>clean(row?.id);
const rowCardId=row=>clean(row?.cardId??row?.card_id);
const rowGameId=row=>clean(row?.gameId??row?.game_id??row?.game);
const rowProviderId=row=>clean(row?.providerId??row?.provider_id);
const rowCardNumber=row=>clean(row?.officialCardNumber??row?.official_card_number??row?.localCardNumber??row?.local_card_number);
const rowImageUrl=row=>clean(row?.imageUrl??row?.image_url);
const rowChineseName=row=>clean(row?.nameZh??row?.name_zh);

/**
 * Audit catalog relationships without guessing identity from names. The
 * returned shape is intentionally JSON-safe so it can be attached to public
 * coverage responses or written to a sync-run metadata object.
 */
export function auditCatalogLinks({cards=[],printings=[],images=[]}={}){
  const cardRows=(cards||[]).filter(row=>row&&typeof row==='object'),cardIds=new Set(cardRows.map(rowId).filter(Boolean)),cardById=new Map(cardRows.map(row=>[rowId(row),row])),printingRows=(printings||[]).filter(row=>row&&typeof row==='object'),imageRows=(images||[]).filter(row=>row&&typeof row==='object');
  const printingsWithKnownCard=printingRows.filter(row=>cardIds.has(rowCardId(row))),imagesWithKnownCard=imageRows.filter(row=>cardIds.has(rowCardId(row))),cardsWithPrintings=new Set(printingsWithKnownCard.map(rowCardId)),cardsWithImages=new Set([...imagesWithKnownCard,...printingsWithKnownCard.filter(rowImageUrl).map(rowCardId)]),stableIdentityRows=cardRows.filter(row=>rowId(row)&&rowGameId(row)&&(rowProviderId(row)||rowCardNumber(row))),missingImages=cardRows.filter(row=>!cardsWithImages.has(rowId(row))&&!rowImageUrl(row)),missingChineseNames=cardRows.filter(row=>!rowChineseName(row)),orphanPrintings=printingRows.filter(row=>!cardIds.has(rowCardId(row))),orphanImages=imageRows.filter(row=>!cardIds.has(rowCardId(row))),crossGameReferences=printingRows.filter(row=>{const card=cardById.get(rowCardId(row)),printingGame=rowGameId(row);return Boolean(card&&printingGame&&rowGameId(card)&&printingGame!==rowGameId(card))});
  const providerKeys=new Map();for(const row of cardRows){const provider=rowProviderId(row),game=rowGameId(row);if(!provider||!game)continue;const key=`${game}\u001f${provider}`,list=providerKeys.get(key)||[];list.push(rowId(row));providerKeys.set(key,list)}
  const duplicateProviderKeys=[...providerKeys.entries()].filter(([,ids])=>ids.length>1).map(([key,ids])=>({key:key.replace('\u001f',':'),cardIds:ids}));
  const sample=rows=>rows.map(row=>rowId(row)||rowCardId(row)).filter(Boolean).slice(0,50);
  return {
    status:'observed',
    cards:cardRows.length,
    stableIdentityCards:stableIdentityRows.length,
    unkeyedCards:cardRows.length-stableIdentityRows.length,
    cardsWithPrintings:cardsWithPrintings.size,
    cardsWithImages:cardsWithImages.size,
    printingReferences:printingRows.length,
    linkedPrintingReferences:printingsWithKnownCard.length,
    imageReferences:imageRows.length,
    linkedImageReferences:imagesWithKnownCard.length,
    orphanPrintings:orphanPrintings.length,
    orphanImages:orphanImages.length,
    crossGameReferences:crossGameReferences.length,
    duplicateProviderKeys,
    missingImages:{count:missingImages.length,sampleIds:sample(missingImages)},
    missingChineseNames:{count:missingChineseNames.length,sampleIds:sample(missingChineseNames)},
    complete:cardRows.length>0&&stableIdentityRows.length===cardRows.length&&orphanPrintings.length===0&&orphanImages.length===0&&crossGameReferences.length===0
  };
}
...
async function syncPokemonZhTw(db){
  ...
  try{
    ...
    for(const batch of chunks(cards||[],150)){
      const normalized=batch.map(c=>{const id=String(c.id),setId=setIds.find(value=>id.toLocaleLowerCase().startsWith(`${value.toLocaleLowerCase()}-`))||id.split('-')[0],seriesId=seriesById.get(setId.toLocaleLowerCase())||null,imageUrl=c.image?`${c.image}/low.webp`:null,rawRarity=c.rarity??c.rarityLabel??null,card={id:`pokemon-tcgdex-tw-${slug(c.id)}`,canonical_id:`pokemon-tcgdex-tw-${slug(c.id)}`,game_id:'pokemon',series_id:seriesId,official_card_number:c.localId||c.id,...cardRarityFields('pokemon',rawRarity),name_zh:c.name||null,name_ja:null,name_en:null,name_ko:null,aliases:[c.id].filter(Boolean),source:provider,provider_id:c.id,search_text:searchText(c.name,c.localId,c.id,c.setId,rawRarity),metadata:rarityMetadata('pokemon',rawRarity,{tcgdexId:c.id}),updated_at:new Date().toISOString()};nameRows.push(...cardNameRows(card,{source:provider,sourceUrl:`https://www.tcgdex.net/database/cards/${encodeURIComponent(c.id)}`}));rarityRows.push(rarityRow('pokemon',rawRarity,{source:provider,sourceUrl:CARD_NAME_SOURCE_URLS[provider],metadata:{setCode:setId,cardId:c.id}}));return {...c,setId,seriesId,imageUrl,rawRarity,card}});
      stats.written+=await upsert(db,'/tcg_cards',normalized.map(c=>c.card));
      stats.written+=await upsert(db,'/tcg_printings',normalized.map(c=>({card_id:c.card.id,series_id:c.seriesId,region:'TW',language:'zh-Hant-TW',local_set_code:c.setId,local_card_number:c.localId||c.id,...rarityFields('pokemon',c.rawRarity),image_url:c.imageUrl,source_url:`https://www.tcgdex.net/database/cards/${encodeURIComponent(c.id)}`,release_date:null,source:provider,provider_id:c.id,image_rehost_required:false,data_status:c.rawRarity?'verified':'incomplete',source_locale:'zh-Hant-TW',metadata:rarityMetadata('pokemon',c.rawRarity,{imageBase:c.image||null}),updated_at:new Date().toISOString()})),'source,provider_id');stats.seen+=batch.length;stats.cursor=batch.at(-1)?.id||stats.cursor;
    }
  ...
  try{
    ...
    for(const batch of chunks(cards,100)){
      ...
      for(const {c,descriptors} of descriptorsByCard){
        const baseId=`yugioh-ygoprodeck-${c.id}`,baseRarity=c.card_sets?.[0]?.set_rarity,baseCard={id:baseId,canonical_id:`yugioh-ygoprodeck-${c.id}`,game_id:'yugioh',series_id:seriesByName.get(c.card_sets?.[0]?.set_name)||null,official_card_number:String(c.id),...cardRarityFields('yugioh',baseRarity),name_zh:null,name_ja:null,name_en:c.name,name_ko:null,aliases:[c.archetype].filter(Boolean),source:provider,provider_id:String(c.id),search_text:searchText(c.name,c.id,c.archetype,(c.card_sets||[]).flatMap(s=>[s.set_name,s.set_code,s.set_rarity])),metadata:rarityMetadata('yugioh',baseRarity,{type:c.type||null,race:c.race||null,attribute:c.attribute||null}),updated_at:new Date().toISOString()};
        ...
        for(const descriptor of descriptors){
          ...
          printings.push({card_id:baseId,series_id:seriesByName.get(descriptor.setName)||null,region:'US',language:'en-US',local_set_code:descriptor.setName,local_card_number:descriptor.setCode,...rarityFields('yugioh',descriptor.rarity),image_url:null,source_url:`https://db.ygoprodeck.com/card/?search=${encodeURIComponent(c.name)}`,release_date:null,source:provider,provider_id:providerId,image_rehost_required:true,data_status:descriptor.rarity?'verified':'incomplete',source_locale:'en-US',metadata:rarityMetadata('yugioh',descriptor.rarity,{providerImageUrl:descriptor.imageUrl||null,imageId:descriptor.imageId||null,setPriceUsd:descriptor.set.set_price||null,variantKey:variantToken}),updated_at:new Date().toISOString()});
        }
      }
...
async function syncOnePiece(db){
  ...
  try{
    ...
    const cardRows=cards.map(c=>({id:`onepiece-official-${slug(c.providerId)}`,canonical_id:`onepiece-${slug(c.number)}`,game_id:'onepiece',series_id:null,official_card_number:c.number,...cardRarityFields('onepiece',c.rarity),name_zh:null,name_ja:null,name_en:c.name,name_ko:null,aliases:[],source:provider,provider_id:c.providerId,search_text:searchText(c.name,c.number,c.rarity,c.cardType),metadata:rarityMetadata('onepiece',c.rarity,{cardType:c.cardType}),updated_at:new Date().toISOString()}));
    stats.written+=await upsert(db,'/tcg_cards',cardRows);
    const seriesByCode=new Map(series.map(s=>[s.official_code.replace('-',''),s.id])),printingRows=cards.map(c=>({card_id:`onepiece-official-${slug(c.providerId)}`,series_id:seriesByCode.get((c.number.split('-')[0]||'').replace('-',''))||null,region:'ASIA',language:'en',local_set_code:c.number.split('-')[0]||'unknown',local_card_number:c.number,...rarityFields('onepiece',c.rarity),image_url:c.imageUrl,source_url:c.sourceUrl,release_date:null,source:provider,provider_id:c.providerId,image_rehost_required:false,metadata:rarityMetadata('onepiece',c.rarity,{cardType:c.cardType}),updated_at:new Date().toISOString()}));
    ...
    const twCardRows=twCards.map(c=>{const en=englishByProviderId.get(c.providerId),rawRarity=c.rarity||en?.rarity||null;return {id:`onepiece-official-${slug(c.providerId)}`,canonical_id:`onepiece-${slug(c.number)}`,game_id:'onepiece',series_id:seriesByCode.get((c.number.split('-')[0]||'').replace('-',''))||null,official_card_number:c.number,...cardRarityFields('onepiece',rawRarity),name_zh:c.name,name_ja:null,name_en:en?.name||null,name_ko:null,aliases:[c.name,en?.name].filter(Boolean),source:provider,provider_id:c.providerId,search_text:searchText(c.name,en?.name,c.number,c.rarity,c.cardType),metadata:rarityMetadata('onepiece',rawRarity,{cardType:c.cardType}),updated_at:new Date().toISOString()}});
    stats.written+=await upsert(db,'/tcg_cards',twCardRows);
    stats.written+=await upsert(db,'/tcg_printings',twCards.map(c=>({card_id:`onepiece-official-${slug(c.providerId)}`,series_id:twSeriesByCode.get((c.number.split('-')[0]||'').replace('-',''))||null,region:'TW',language:'zh-Hant-TW',local_set_code:c.number.split('-')[0]||'unknown',local_card_number:c.number,...rarityFields('onepiece',c.rarity),image_url:c.imageUrl,source_url:c.sourceUrl,release_date:null,source:'onepiece-official-tw',provider_id:c.providerId,image_rehost_required:false,metadata:rarityMetadata('onepiece',c.rarity,{cardType:c.cardType}),updated_at:new Date().toISOString()})),'source,provider_id');
