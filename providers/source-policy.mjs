...
export function imageRightsAllowDisplay(status,expiresAt=null){
  ...
  if(!DISPLAYABLE_IMAGE_RIGHTS.has(normalized)&&!(registry.unverifiedImageDisplayEnabled&&normalized==='not-provided'))return false;
...
export function sourcePolicySummary(){
  ...
  return {version:registry.version,reviewedAt:registry.reviewedAt,enabled:runtime.filter(source=>source.collectionEnabled).length,blocked:runtime.filter(source=>!source.collectionEnabled).length,total:runtime.length,unverifiedImageDisplayEnabled:registry.unverifiedImageDisplayEnabled===true,riskAcceptedAt:registry.riskAcceptedAt||null};
}
