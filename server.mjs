...
const server=createServer(async(req,res)=>{const url=new URL(req.url,`http://${req.headers.host}`);
  ...
  if(url.pathname==='/api/admin/catalog/sync'&&req.method==='POST'){if(!adminAuthorized(req))return json(res,401,{error:'未授權；管理密鑰不可放在網址'});if(!supabaseConfigured())return json(res,503,{error:'尚未設定資料庫'});const requested=(url.searchParams.get('provider')||'all').toLowerCase(),aliases={pokemonjp:'pokemonJp',pokemonja:'pokemonJp',tcgdexja:'pokemonJp',pokemontw:'pokemonZhTw',pokemonzh:'pokemonZhTw'},selected=aliases[requested]||requested,candidates=requested==='all'?['pokemonJp','pokemonZhTw','pokemon','onepiece','yugioh']:[selected];if(candidates.some(p=>!['pokemonJp','pokemonZhTw','pokemon','onepiece','yugioh'].includes(p)))return json(res,400,{error:'不支援的 provider'});const providers=filterAllowedCatalogProviders(candidates),blocked=candidates.filter(provider=>!providers.includes(provider));if(!providers.length)return json(res,409,{error:'所選來源尚未通過來源政策',blocked});syncCatalog(supabaseFetch,{providers}).then(result=>console.log('catalog sync complete',JSON.stringify(result))).catch(error=>console.error('catalog sync failed',error));return json(res,202,{data:{message:'Catalog 同步已在背景開始',providers,blocked}})}
