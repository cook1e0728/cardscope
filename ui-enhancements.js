...
function openCardZoom(source,alt,options={}){
  ...
  const fitMediaToViewport=()=>{if(!media||!img?.naturalWidth||!img?.naturalHeight)return;const size=viewportSize(),ratio=img.naturalWidth/img.naturalHeight,width=Math.min(size.width,size.height*ratio),height=width/ratio;media.style.width=`${Math.max(1,width).toFixed(2)}px`;media.style.height=`${Math.max(1,height).toFixed(2)}px`};
  img.onload=()=>{img.hidden=false;if(fallback)fallback.hidden=true;if(sample)sample.hidden=false;fitMediaToViewport();if(quality){const lowResolution=img.naturalWidth<700;quality.textContent=`來源 ${img.naturalWidth}×${img.naturalHeight}${lowResolution?' · 低解析':''}`;quality.classList.toggle('is-low-resolution',lowResolution)}applyZoom()};
