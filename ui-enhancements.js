...
function closeCardZoom(){const zoom=$('cardZoom');if(zoom)zoom.classList.remove('open')}
function openCardZoom(src,alt){
  if(!src)return;
  let zoom=$('cardZoom');
  if(!zoom){zoom=document.createElement('div');zoom.id='cardZoom';zoom.className='card-zoom';zoom.setAttribute('role','dialog');zoom.setAttribute('aria-modal','true');document.body.append(zoom)}
  zoom.innerHTML=`<button type="button" class="card-zoom-close" aria-label="關閉放大卡圖">×</button><img src="${e(src)}" alt="${e(alt)}放大卡圖">`;
  zoom.classList.add('open');zoom.onclick=event=>{if(event.target===zoom||event.target.closest('.card-zoom-close'))closeCardZoom()};zoom.querySelector('.card-zoom-close')?.focus();
}
...
function activateDetailInteractions(){
  ...
  document.querySelectorAll('.tilt-card').forEach(tilt=>{
    ...
    if(tilt.dataset.cardZoomSrc){tilt.classList.add('can-zoom');tilt.onclick=()=>openCardZoom(tilt.dataset.cardZoomSrc,tilt.dataset.cardZoomAlt);tilt.onkeydown=event=>{if(event.key==='Enter'||event.key===' '){event.preventDefault();openCardZoom(tilt.dataset.cardZoomSrc,tilt.dataset.cardZoomAlt)}}}
  });
}
