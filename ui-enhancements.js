...
function activateDetailInteractions(){
  ...
  document.querySelectorAll('.tilt-card').forEach(tilt=>{
    ...
    if(tilt.dataset.cardZoomSrc||tilt.dataset.cardZoomSources){const sources=()=>cardZoomSourcesFromElement(tilt);tilt.classList.add('can-zoom');tilt.onclick=()=>openCardZoom(sources(),tilt.dataset.cardZoomAlt);tilt.onkeydown=event=>{if(event.key==='Enter'||event.key===' '){event.preventDefault();openCardZoom(sources(),tilt.dataset.cardZoomAlt)}}}
  });
}
...
openCard=async function(id){
  ...
  try{
    ...
    const zoomSourceAttribute=detailImageSources.length?`data-card-zoom-src="${e(detailImageSources[0])}" data-card-zoom-sources="${e(JSON.stringify(detailImageSources))}" data-card-zoom-alt="${e(name(c))}"`:'';
...
const interactiveZoomStyle=document.createElement('style');interactiveZoomStyle.textContent=`body.card-zoom-open{overflow:hidden}.card-zoom{display:none;place-items:stretch;padding:0;background:#090909ed;color:#fff;cursor:default;overscroll-behavior:contain}.card-zoom.open{display:block}.card-zoom-viewport{position:absolute;inset:60px 0 70px;display:grid;place-items:center;overflow:hidden;touch-action:none;cursor:grab;outline:none}.card-zoom-viewport:active{cursor:grabbing}.card-zoom-viewport:focus-visible{box-shadow:inset 0 0 0 3px var(--y)}.card-zoom-canvas{position:relative;width:min(900px,92vw);height:min(82vh,calc(100vh - 140px));display:grid;place-items:center;overflow:visible}.card-zoom-image{display:block;width:100%;height:100%;max-width:none;max-height:none;object-fit:contain;transform-origin:center center;user-select:none;-webkit-user-drag:none;will-change:transform;filter:drop-shadow(0 24px 45px #000)}.card-zoom-fallback{display:grid;place-items:center;text-align:center;color:#ddd;font-size:18px;line-height:1.6}.card-zoom-fallback small{font-size:12px;color:#aaa}.card-zoom-toolbar{position:fixed;left:50%;bottom:17px;z-index:2;display:flex;align-items:center;gap:4px;max-width:calc(100vw - 24px);padding:7px;border:1px solid #fff3;border-radius:13px;background:#151515ee;box-shadow:0 10px 30px #0008;transform:translateX(-50%);color:#fff}.card-zoom-toolbar button{border:1px solid #fff3;border-radius:8px;background:#292929;color:#fff;padding:7px 10px;cursor:pointer;white-space:nowrap}.card-zoom-toolbar button:hover,.card-zoom-toolbar button:focus-visible{background:#444;border-color:#fff8}.card-zoom-toolbar output{min-width:72px;padding:0 4px;text-align:center;font-size:13px;font-variant-numeric:tabular-nums}.card-zoom-divider{width:1px;height:25px;margin:0 3px;background:#fff4}.card-zoom-close{position:fixed;right:18px;top:14px;z-index:3;width:44px;height:44px;border:1px solid #fff5;border-radius:50%;background:#111;color:#fff;font-size:27px;line-height:1;cursor:pointer}.card-zoom-close:hover,.card-zoom-close:focus-visible{background:#333;outline:2px solid var(--y);outline-offset:2px}@media(max-width:520px){.card-identity{grid-template-columns:1fr}.card-zoom-viewport{inset:58px 0 64px}.card-zoom-canvas{width:94vw;height:calc(100vh - 132px)}.card-zoom-toolbar{bottom:9px;gap:2px;padding:5px}.card-zoom-toolbar button{padding:7px 6px;font-size:12px}.card-zoom-toolbar output{min-width:57px;font-size:12px}.card-zoom-divider{height:21px;margin:0 1px}}@media(prefers-reduced-motion:reduce){.card-zoom-image{transition:none}}`;document.head.append(interactiveZoomStyle);
installFeatureTour();
installViewToggle();
