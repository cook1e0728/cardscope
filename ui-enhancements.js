...
async function installCatalogHealth(){
  ...
  try{
    ...
    if($('status'))$('status').textContent=`資料庫收錄：${Number(body.data?.totalCards||C.cards.length).toLocaleString()} 張卡片；${P.length.toLocaleString()} 筆實體商品。各系列官方總數尚未核定時不顯示完成率。`;
    section.innerHTML=`<summary>資料完整度與圖片權利狀態</summary><div class="health-grid">${Object.entries(labels).map(([id,label])=>{const row=games[id]||{};return`<div class="health-item"><b>${e(label)}｜${Number(row.cards||0).toLocaleString()} 張</b><span>中文名 ${Number(row.chineseNames||0).toLocaleString()} · 稀有度 ${Number(row.rarities||0).toLocaleString()}</span><span>圖片網址 ${Number(row.cardsWithImageUrls??row.displayableImages??0).toLocaleString()} · 可公開顯示 ${Number(row.displayableImages||0).toLocaleString()}</span></div>`}).join('')}</div><p class="health-note">「圖片網址」只表示資料來源有提供位置；只有具明確權利狀態且未過期的圖片才會顯示。官方總卡數尚未核定，因此不顯示假完成率。來源政策版本 ${e(body.data?.policy?.version||'—')}，檢查日 ${e(body.data?.policy?.reviewedAt||'—')}。</p>`;
